...
`DatabaseStats.cs` contains the `Database` class (the one non-partial-MatchZy class of note). It supports **SQLite (default) and MySQL**, selected via `cfg/MatchZy/database.json` (`DatabaseType` field). Queries use Dapper; table DDL is duplicated as `CreateRequiredTablesSQLite()` / `CreateRequiredTablesSQL()` (SQLite vs MySQL dialect) — **add schema changes to both**. Tables: `matchzy_stats_matches`, `matchzy_stats_players`, `matchzy_stats_maps`. Per-match detailed stats are also written to CSV (CsvHelper). Demos/backups can be uploaded over HTTP (see `DemoManagement.cs`, `BackupManagement.cs`, `RemoteLogConfig.cs`).
