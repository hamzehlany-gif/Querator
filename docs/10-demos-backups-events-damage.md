...
`!isMatchLive || isRoundRestoring`). Writes `csgo/MatchZyDataBackup/querator_<id>_<map>_round<NN>.json` — a
...
tactical timeout, and — if `querator_stop_command_no_damage` is set — if `playerHasTakenDamage` this round. T sets
...
Builds `querator_<id>_<map>_round<NN>.json` and calls **`RestoreRoundBackup`** — the core restore engine:
...
zipped**). Restore-from-remote: `querator_loadbackup`/`get5_loadbackup` (file), `querator_loadbackup_url`/
`get5_loadbackup_url` (URL → saved as `MatchZyDataBackup/<GUID>.json` → restore), `querator_listbackups`/
...
- **`SendEventAsync(QueratorEvent)`**: if `matchConfig.RemoteLogURL` is empty, **no-op**. Else serializes the event to
  ...
  `Task.Run`). Configured via `querator_remote_log_url`/`get5_remote_log_url` (+ header key/value) — see
...
The "To/From" report printed at the end of each round (and after knife). Gated by `querator_enable_damage_report`.
