...
- **Usage:** `Localizer["querator.<key>", arg0, arg1, …]` (CSSharp `BasePlugin.Localizer`). Values use `{0}`,`{1}`…
  placeholders and inline color tokens (`{green}`, `{red}`, `{default}`, …) that resolve to `ChatColors`.
- **Key namespaces** (the `querator.<area>.<name>` convention):
...
- **Adding a user-facing string:** add the key to `lang/en.json` (and ideally the other locales), then use
  `Localizer["querator.<area>.<name>", …]`. **Don't hardcode** player-facing strings.
...
| `live.cfg` | live (5v5) | competitive ruleset; **ends with `exec Querator/live_override.cfg`**. |
| `live_wingman.cfg` | live (wingman) | 2v2 ruleset; ends with `exec Querator/live_wingman_override.cfg`. |
...
querator_chat_prefix                    [{Green}Querator{Default}]
...
querator_hostname_format                "Querator | {TEAM1} vs {TEAM2}"
querator_enable_damage_report           true
querator_match_start_message            ""
```
(Header comment says "Do not add commands other than querator config console variables." Full per-ConVar reference:
[04](04-commands-and-convars.md#6-convars).)

### JSON / data files (in `cfg/Querator/`)
| File | Shape | Purpose |
|---|---|---|
| `admins.json` | `{ "<steam64>": "<role>" }` | Querator's own admin list (empty role = full admin). Auto-created with a `{"steamid":""}` placeholder if missing. Loaded by `LoadAdmins`; reload with `.reload_admins`. |
...
- The plugin's identity spans the `querator_*` ConVar names, the `Querator/` cfg folder name, the `querator.*` lang keys,
  and the `chatPrefix` defaults — any future identity change must be coordinated across all of them at once (see [02](02-build-test-deploy.md#6-fork-specific-gotchas-to-remember)).
