...
(applies `matchzy_hostname_format`), `LoadClientNames`/`WriteClientNamesInFile` (writes a `Match_<id>.ini` name file),
...
| File | When | Notes |
|---|---|---|
| `config.cfg` | plugin load | Sets all the default `matchzy_*` ConVars (§3a). |
...
- Renaming the plugin touches the `matchzy_*` ConVar names, the `MatchZy/` cfg folder name, the `matchzy.*` lang keys,
